...
int main(int argc, char** argv) {
    ...
    func_handler _create_block_manager = dlsym(dl_handle, "create_block_manager");
    func_handler _add_edit_block = dlsym(dl_handle, "add_edit_block");
    func_handler _create_diff_file = dlsym(dl_handle, "create_diff_file");
    func_handler _delete_operation = dlsym(dl_handle, "delete_operation");
    func_handler _delete_edit_block = dlsym(dl_handle, "delete_edit_block");
    ...
    while (i < argc) {
        if (strcmp(argv[i], "create_table") == 0) {
            ...
            manager = _create_block_manager(blocks);
            ...
            while (i < argc && !is_command(argv[i])) {
                ...
                char* tmp_file = _create_diff_file(firstfile, secondfile);
                _add_edit_block(manager, tmp_file);
                i++;
            }
            ...
            int idx = atoi(argv[i]);
            real_time[0] = times(&tms_time[0]);
            _delete_edit_block(manager, idx);
            real_time[1] = times(&tms_time[1]);
            ...
            real_time[0] = times(&tms_time[0]);
            _delete_operation(manager, block_idx, op_idx);
            real_time[1] = times(&tms_time[1]);
