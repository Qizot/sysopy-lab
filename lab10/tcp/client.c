...
void init(char* name, char* connection_type, char* server_addr, char* server_port) {
    ...
    send_message(client_socket, &msg, sizeof(msg));
}
...
_Noreturn void handle_messages() {
    ...
    for(;;) {
        void* buf = read_message(client_socket, &type);
        ...
        switch (type) {
            ...
            case START_GAME: {
                start_game_message_t *msg = buf;
                client_character = msg->character;
                oponent_character = msg->character == 'X' ? 'O' : 'X';
            ...
            case MOVE: {
                ...
                board[msg->field] = oponent_character;
                is_clients_move = 1;
            ...
            default: {
                FATAL_ERROR("Received unknown message type: %d", type);
            }
        }
...
void start_game(void* _) {
    for (;;) {
        ...
        if ((result = is_game_finished()) != -1) {
            ...
            send_message(client_socket, &action, sizeof(action));
            pthread_mutex_unlock(&mutex);
            return;
        }
